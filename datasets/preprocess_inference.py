...
if __name__ == '__main__':
    imgdir = '/home/lyf/Research/auto_trace/neuronet/data/test_data/for_shendian/tiff'

    samples = []
    for imgfile in glob.glob(os.path.join(imgdir, '*.tiff')):
        print(imgfile)
        image = sitk.GetArrayFromImage(sitk.ReadImage(imgfile)).astype(np.float32)
        if image.ndim == 3:
            image = image[None]
        # normalize the image
        image = normalize_normal(image, mask=None)
        # write the image and tree as well
        imgfile_out = f'{os.path.splitext(imgfile)[0]}.npz'
        np.savez_compressed(imgfile_out, data=image.astype(np.float32))
        samples.append((imgfile_out, None, np.array([1.0, 0.23, 0.23])))
